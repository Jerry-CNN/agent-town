...
### Backend Logging
Every LLM-touching code path must log at INFO level so the developer can follow agent behavior in the terminal. At minimum, log:
- **Agent step start/end**: which agent, what phase (perceive/move/converse/decide)
- **LLM calls**: model used, prompt type (schedule/decide/converse/arena), latency
- **Decisions**: agent name, chosen destination, activity, whether gating skipped the call
- **Conversations**: who talked to whom, turn count, termination reason
- **Errors/fallbacks**: when a fallback action is used instead of a real LLM response

Use `logger.info()` for the above (visible by default). Reserve `logger.debug()` for internal details like semaphore acquire/release. The goal: a developer watching the terminal should understand what every agent is doing each tick without needing to click agents in the UI.
<!-- GSD:conventions-end -->
